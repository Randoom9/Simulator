...
if (isset($_POST['done'])) {
	if ($_POST['answer'] == "") {
		echo "";
	} else if (trim($_POST['answer']) == "2(3+4)=14") {
		header("Location:../six/end.php");
	} else if (trim($_POST['answer']) == "2*(3+4)=14") {
		header("Location:../six/end.php");
		} else {
		$err = "error";
...
.name	h1 {
	...
	<div class="wrapper">
		...
		<div class="pillar hide" style="min-height: 600px; width:1040px;">
			<div class="name">
			...
			<br>
			<div class="task">
				<img src="img/yrta.gif" alt="Юрта">

				<p><span>Задание№2:</span><span class="red"></span></p>
				<p><span>Дано:</span>юрта, диаметром D=4м.Высота стен равна половине ее диаметра. Считать крышу - полусферой.</p>
				<p>1.Составьте и напишите формулу периметра контура юрты (вид спереди)</p>
				<p>P<sub>ABCDE</sub>=<form style="position: relative; top: 50px; left: 250px;" action="" method="post"><input type="text" autocomplete="off" name="answer">M<input type="submit" name="done"></form><p style="position: relative; top: 25px; color: red;"><?php if(isset($err)){echo "Ошибка";}; ?></p></p>
			...
			<div class="buttons">
				...
				<button><a href="../fourth/perimetr3.php">Назад</a></button>
